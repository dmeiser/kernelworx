 ...
 * 
 * Usage:
 * 1. Call trackResource() whenever you create a resource (profile, campaign, order, etc.)
...
interface TrackedResource {
  type: 'profile' | 'campaign' | 'order' | 'catalog' | 'share' | 'invite';
  id: string; // shareId for shares, targetAccountId stored separately
  parentId?: string; // profileId for campaigns/orders/shares/invites, null for profiles/catalogs
...
export async function cleanupAllTrackedResources(suiteId: string): Promise<void> {
  ...
  // Delete child resources first (orders, campaigns, shares, invites)
