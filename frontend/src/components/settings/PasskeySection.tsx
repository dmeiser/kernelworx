...
const PasskeyStatusAlerts: React.FC<{ hook: UsePasskeysReturn }> = ({
  hook,
}) => (
  <>
    {hook.passkeySuccess ? (
      <Alert
        severity="success"
        sx={{ mb: 2 }}
        onClose={() => hook.setPasskeySuccess(false)}
      >
    ...
    {hook.passkeyError ? (
      <Alert
        severity="error"
        sx={{ mb: 2 }}
        onClose={() => hook.setPasskeyError(null)}
      >
        {hook.passkeyError}
      </Alert>
    ) : null}
...
const RegisteredPasskeys: React.FC<{ hook: UsePasskeysReturn }> = ({
  hook,
}) => {
          ...
          <ListItem
            ...
            secondaryAction={
              <IconButton
                edge="end"
                onClick={() =>
                  pk.credentialId && hook.handleDeletePasskey(pk.credentialId)
                }
  ...
  <Box>
    ...
    <Stack direction="row" spacing={2} alignItems="flex-start">
      ...
      <Button
        variant="contained"
        startIcon={
          hook.passkeyLoading ? <CircularProgress size={20} /> : <AddIcon />
        }
