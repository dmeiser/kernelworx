...
const MfaConflictWarning: React.FC<MfaConflictWarningProps> = ({
  show,
  passkeyCount,
}) => {
  if (!show) return null;
  ...
  return (
    <Alert severity="warning" sx={{ mb: 2 }}>
      ...
      {passkeyCount > 1 ? "s" : ""} registered. TOTP MFA and Passkeys cannot be
      used together. Enabling MFA will delete all your passkeys.
    </Alert>
  );
};
  ...
  <>
    {hook.mfaSuccess && (
      <Alert
        severity="success"
        sx={{ mb: 2 }}
        onClose={() => hook.setMfaSuccess(false)}
      >
...
const MfaPrimaryActions: React.FC<MfaPrimaryActionsProps> = ({
  hook,
  onSetup,
}) => (
  <>
      ...
      <form onSubmit={hook.handleVerifyMFA}>
        <Stack direction="row" spacing={2} alignItems="flex-start">
          <TextField
            ...
            onChange={(event) =>
              hook.setMfaVerificationCode(
                event.target.value.replace(/\D/g, "").substring(0, 6),
              )
            }
            required
            disabled={hook.mfaLoading}
          ...
          <Button
            type="submit"
            variant="contained"
            disabled={hook.mfaLoading || hook.mfaVerificationCode.length !== 6}
          >
            {hook.mfaLoading ? (
...
export const MfaSection: React.FC<MfaSectionProps> = ({
  ...
  <Paper sx={{ p: 3, mb: 3 }}>
    ...
    <MfaConflictWarning
      show={passkeyCount > 0 && !mfaHook.mfaEnabled}
