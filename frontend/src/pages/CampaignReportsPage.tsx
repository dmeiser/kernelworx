...
const buildSellerReportWorkbook = (
  report: UnitReport,
  productList: string[],
) => {
  ...
  const dataRows = report.sellers.map((seller) => {
    const { totals, totalItems } = calculateSellerProductTotals(
      seller,
      productList,
    );
    return [
  ...
  const { grandTotals, grandTotalItems } = calculateGrandTotals(
    report,
    productList,
  );
  const totalsRow = [
...
const buildOrderDetailsWorkbook = (
  report: UnitReport,
...
const CampaignSelectorCard: React.FC<CampaignSelectorProps> = ({
  ...
  if (!campaigns.length) {
    return (
      <CampaignSelectorShell>
        <Alert severity="info">
          You don't have any active shared campaigns yet. Create one to start
          generating reports.
        </Alert>
      </CampaignSelectorShell>
    );
...
const CampaignSelectorShell: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => (
  <Paper sx={{ p: 3 }}>
    <Stack spacing={2}>
  ...
  <Stack spacing={2}>
    ...
    >
      {campaigns.map((campaign) => (
        ...
        >
          {campaign.unitType} {campaign.unitNumber} - {campaign.campaignName}{" "}
          {campaign.campaignYear} ({campaign.catalog.catalogName})
    ...
    <Button
      ...
      startIcon={
        loadingReport ? <CircularProgress size={20} /> : <ReportIcon />
      }
    >
...
const CampaignDetails: React.FC<{ campaign: SharedCampaign }> = ({
  campaign,
}) => (
...
const ReportViewSelector: React.FC<{
  ...
  <Stack direction="row" spacing={1}>
    <Button
      variant={view === "unit" ? "contained" : "outlined"}
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={3}>
        ...
        <Box flex={1}>
          ...
          <Typography variant="h4">
            {formatCurrency(report.totalSales / report.sellers.length)}
          </Typography>
        </Box>
      </Stack>
            ...
            {topSellers.map((seller, idx) => {
              ...
              return (
                <TableRow key={seller.profileId}>
                  ...
                  <TableCell align="right">
                    {((seller.totalSales / report.totalSales) * 100).toFixed(1)}
                    %
                  </TableCell>
                </TableRow>
              );
...
const useCampaignReportState = () => {
  ...
  const { report, loading, error, refetch } = useUnitReport(
    selectedCampaign,
    canGenerateReport,
  );
  const { productList, allOrders, allProducts, totalItems, topSellers } =
    useReportDerivatives(report);
...
const useSharedCampaignSelection = () => {
  ...
  const campaigns = useMemo(
    () => getActiveCampaigns(sharedCampaignsData),
    [sharedCampaignsData],
  );
  ...
  const selectedCampaign = useMemo(
    () =>
      campaigns.find(
        (campaign) =>
          campaign.sharedCampaignCode === selectedSharedCampaignCode,
      ),
    [campaigns, selectedSharedCampaignCode],
  );
...
const createExportSellerHandler = (
...
const createExportOrderHandler = (
  report: UnitReport | undefined,
  allOrders: SellerOrder[],
  allProducts: string[],
) => {
...
const ReportHeaderCard: React.FC<{
  ...
  <Paper sx={{ p: 3 }}>
    <Stack spacing={2}>
      ...
      {!hasSellers && (
        <Alert severity="info">
          No sales data found for this unit in {report.campaignYear}. Make sure
          sellers have set their unit type and number on their profiles.
        </Alert>
      )}
    </Stack>
  </Paper>
