...
import { request, response } from './get_shared_campaign_catalog_fn.js';

describe('get_shared_campaign_catalog_fn request', () => {
    it('skips the GetItem when the catalog was batch-resolved by a parent pipeline', () => {
        ...
        const ctx = {
            prev: { result: catalog },
            source: { sharedCampaignCode: 'SC1', catalogId: 'CAT-A' },
        };
    ...
    it('performs the singular GetItem when the source has no batch-resolved catalog', () => {
        const ctx = { source: { sharedCampaignCode: 'SC1', catalogId: 'CAT-A' } };
...
describe('get_shared_campaign_catalog_fn response', () => {
    it('returns the catalog item when present and active', () => {
        const catalog = { catalogId: 'CAT-A', catalogName: 'Fall Sale' };
    ...
    it('returns null for a soft-deleted catalog (unchanged VTL contract)', () => {
        const ctx = { result: { catalogId: 'CAT-A', isDeleted: true } };

        assert.strictEqual(response(ctx), null);
    });

    it('coerces a legacy String isPublic to Boolean before serialization', () => {
        const ctx = { result: { catalogId: 'CAT-A', isPublic: 'true' } };

        assert.deepStrictEqual(response(ctx), { catalogId: 'CAT-A', isPublic: true });
    });
