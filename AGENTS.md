...
- update pipeline: `refresh_latest_campaign_lookup` + `refresh_latest_campaign_write` recompute when `input.isActive` is present; they no-op otherwise (`runtime.earlyReturn`).
...
Read paths: `listMyProfiles` is a pipeline (`list_my_profiles` → `batch_latest_campaigns`, one `BatchGetItem` per page via the `templatefile` `${campaigns_table_name}` pattern); its batch step only annotates profiles that have the pointer AND an active fetched campaign — unmigrated profiles (no `latestCampaignId`) fall through to the field resolver, which keeps the original per-item Query as the fallback. `getProfile`/`adminGetUserProfiles` return raw items so they use the field resolver's GetItem fast path automatically; `listMyShares` must copy `latestCampaignId` explicitly in `_build_shared_profile_result` (`src/handlers/profile_sharing.py`). Existing rows need no backfill: they are correct via the fallback and migrate on their next campaign mutation. When adding a campaign-creating mutation, wire the pointer SET into the same pipeline or the field drifts.
