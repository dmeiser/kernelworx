...
The AppSync JS runtime (APPSYNC_JS 1.0.0) is not full ES2020: the `in` operator fails every `CreateFunction`/`UpdateFunction` with `BadRequestException: The code contains one or more errors.` (#332 shipped `in` and the ephemeral deploy died on it). Node-based unit tests pass regardless, so this only surfaces at deploy time — avoid `in` (use an `undefined`/hasOwnProperty check) and diff `dist/` against what previously deployed when adding new language features.
