...
### Temporary: legacy-OAI destroy-ordering scaffold (#335 deploy fix)

`#335`/`#359` removed `aws_cloudfront_origin_access_identity.main` from `tofu/application/modules/cloudfront/main.tf`, leaving a dangling destroy that failed deploys with `CloudFrontOriginAccessIdentityInUse` (the delete raced the OAC cutover; CloudFront's InUse check runs against the live config, which only stops referencing the OAI once the update Deployed). Until a follow-up removes it after the OAI is destroyed in all environments, the module carries a one-time scaffold: `terraform_data.legacy_oai_destroy_gate` (create-time local-exec polling `aws cloudfront get-distribution --query 'Distribution.Status'` until `Deployed`, ≤30 min) orders both the re-added `count = 0` OAI destroy and the `aws_s3_bucket_policy.static` principal flip (`depends_on` on both). Do not delete the `count = 0` block or relax the `count == 0` allowance in `tests/unit/test_cloudfront_oac.py` while the scaffold exists.
