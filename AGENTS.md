...
Read paths: `listMyProfiles` is a pipeline (`list_my_profiles` → `batch_latest_campaigns`, one `BatchGetItem` per page via the `templatefile` `${campaigns_table_name}` pattern); a page yielding 0 or >100 keys skips the BatchGetItem entirely (BatchGetItem caps at 100 keys and an AppSync function can issue only one call) and passes the connection through unchanged, so every profile on that page falls back to the field resolver instead of failing the query. The batch step only annotates profiles that have the pointer AND an active fetched campaign. Because AppSync always executes an attached field resolver and its result overwrites pipeline-attached values (the #330 `PaymentMethod.qrCodeUrl` trap), `seller_profile_latest_campaign_resolver.js` FIRST short-circuits with `runtime.earlyReturn(ctx.source.latestCampaign)` when the batch attached it (zero datastore calls), then uses a single GetItem when `latestCampaignId` is present, and keeps the original per-item GSI Query only for unmigrated profiles. `getProfile`/`adminGetUserProfiles` return raw items so they use the field resolver's GetItem fast path automatically; `listMyShares` must copy `latestCampaignId` explicitly in `_build_shared_profile_result` (`src/handlers/profile_sharing.py`). Existing rows need no backfill: they are correct via the fallback and migrate on their next campaign mutation. When adding a campaign-creating mutation, wire the pointer SET into the same pipeline or the field drifts.
